...
#define run_rmi_binary(dtype, name, suffix, variant)  if (filename.find("/" #name "_" #dtype) != std::string::npos) { benchmark.template Run<RMI_B<NAME2(dtype, _t), variant, NAME5(name, _, dtype, _, suffix)::BUILD_TIME_NS, NAME5(name, _, dtype, _, suffix)::RMI_SIZE, NAME5(name, _, dtype, _,suffix)::NAME, NAME5(name, _, dtype, _, suffix)::lookup, NAME5(name, _, dtype, _, suffix)::load, NAME5(name, _, dtype, _, suffix)::cleanup>>(); }


#define run_rmi_binary_pareto(dtype, name) {  run_rmi_binary(dtype, name, 0, 0); if (pareto) { run_rmi_binary(dtype, name, 1, 1); run_rmi_binary(dtype, name, 2, 2); run_rmi_binary(dtype, name, 3, 3); run_rmi_binary(dtype, name, 4, 4); if (benchmark.uses_binary_search()) { run_rmi_binary(dtype, name, 5, 5); run_rmi_binary(dtype, name, 6, 6); run_rmi_binary(dtype, name, 7, 7); run_rmi_binary(dtype, name, 8, 8); run_rmi_binary(dtype, name, 9, 9); } } }
...
void benchmark_32_rmi(sosd::Benchmark<uint32_t, Searcher>& benchmark,
  ...
  run_rmi_binary_pareto(uint32, fb_200M);
