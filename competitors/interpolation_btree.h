...
public:
   ...
   uint32_t LowerBound(KeyType key)
   {
      ...
      // Binary search
      //      auto result = std::lower_bound(keys.begin(), keys.begin() + count, key, [](KeyType lhs, KeyType rhs) {
      //         return lhs<rhs;
      //      });
...
uint64_t BTree<Key, Value>::GetSizeRec(const MMBTreeInner *node) const
{
   ...
   for (uint32_t i = 0; i<=node->count; i++) {
      if (node->children[i]->page_type == PageType::kBTreeInner) {
         result += GetSizeRec(reinterpret_cast<const MMBTreeInner *>(node->children[i]));
      }
...
template<class Key, class Value>
uint64_t BTree<Key, Value>::FastLoadGenerate(std::function<bool(Key &, Value &)> generator)
{
   height = 0;
   ...
   while (nextLayerKeys.size()>1) {
      ...
      // Build one layer
      uint64_t pos = 0;
      uint64_t requiredInnerNodes = currentLayerKeys.size() / MMBTreeInner::MAX_ENTRIES + (currentLayerKeys.size() % MMBTreeInner::MAX_ENTRIES == 0 ? 0 : 1);
      uint64_t entriesPerInnerNode = currentLayerKeys.size() / requiredInnerNodes + (currentLayerKeys.size() % MMBTreeInner::MAX_ENTRIES == 0 ? 0 : 1);
      assert(entriesPerInnerNode>1);
      for (uint32_t innerNode = 0; innerNode<requiredInnerNodes; innerNode++) {
         if (innerNode * entriesPerInnerNode + (requiredInnerNodes - innerNode) * (entriesPerInnerNode - 1) == currentLayerKeys.size()) {
            entriesPerInnerNode--;
         }
         ...
         inner->Init();
         nextLayerKeys.push_back(std::make_pair(currentLayerKeys[pos + entriesInThisNode - 1].first, inner));
...
public:
   ...
   {
      return sizeof(*this) + btree_.GetSize();
   }
