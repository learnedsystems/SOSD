...
class ART32Primary : public Competitor {
 public:
  ~ART32Primary() { destructTree(tree_); }
  ...
  uint64_t EqualityLookup(const uint64_t lookup_key) {
    ...
    if (!isLeaf(leaf))
      util::fail("ART32: search ended in inner node");
    const uint64_t offset = getLeafValue(leaf);
  ...
  bool applicable(bool unique,
                  const std::string& data_filename) const { return unique; }
  ...
  void insertNode16(Node16* node,
    ...
    if (node->count < 16) {
      ...
      __m128i cmp = _mm_cmplt_epi8(_mm_set1_epi8(keyByteFlipped),
                                   _mm_loadu_si128(reinterpret_cast<__m128i*>(node->key)));
      uint16_t
          bitfield = _mm_movemask_epi8(cmp) & (0xFFFF >> (16 - node->count));
...
uint64_t ART32Primary::allocated_byte_count;
