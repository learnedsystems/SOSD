...
class ARTPrimary : public Competitor {
 public:
  ~ARTPrimary() { destructTree(tree_); }
  ...
  SearchBound EqualityLookup(const uint64_t lookup_key) {
    ...
    const uint64_t guess = getLeafValue(leaf);
    const uint64_t error = size_scale;
    const uint64_t start = (guess < error ? 0 : guess - error);
    const uint64_t stop = (guess + error >= data_size_ ? data_size_ : guess + error);
  ...
  bool applicable(bool unique,
                  const std::string& _data_filename) const { return true; }
...
template<int size_scale>
uint64_t ARTPrimary<size_scale>::allocated_byte_count;
