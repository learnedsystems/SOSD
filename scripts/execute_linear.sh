...
echo "Executing linear search benchmark and saving results..."
...
function do_benchmark() {
    ...
    RESULTS=./results/$1_results_linear.txt
    if [ -f $RESULTS ]; then
        echo "Already have results for $1"
    else
        echo "Executing workload $1"
        $BENCHMARK -r 1 ./data/$1 ./data/$1_equality_lookups_1M --search linear --pareto | tee $RESULTS
    fi
}
